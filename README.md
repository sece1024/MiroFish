...
MiroFish 支持**小说创作**特化模式。用户提供小说大纲和角色设定，AI 自动演绎剧情并生成完整小说。

### 工作流程

```
用户输入(大纲 + 角色) → StoryPlanner → 场景序列
                            ↓
                    NarrativeSpace → 角色交互日志
                            ↓
                    ChapterWriter → 完整小说(Markdown)
```

1. **故事编排 (StoryPlanner)**：将大纲拆解为章节，再拆解为具体场景（地点、时间、出场角色、核心冲突）
2. **场景推演 (NarrativeSpace)**：在每个场景中，角色 Agent 以小说身份进行多轮交互（对话、内心独白、行为、反应）
3. **章节撰写 (ChapterWriter)**：将角色交互日志转化为优美的小说正文，支持多种风格（武侠/言情/悬疑/科幻/现代等）
